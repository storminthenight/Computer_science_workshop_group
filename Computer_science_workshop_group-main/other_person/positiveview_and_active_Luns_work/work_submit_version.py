...
def main():
    ...
    # read twice to check where the header is (debug )
    df_raw = pd.read_excel(file_path, sheet_name="vgi_game_stats", header=None)

    # based on the raw read, we know the header is on row 1 (0-based index), so we read again with header=1
...
# all graph files have a main() function, so we can run them as scripts if needed
